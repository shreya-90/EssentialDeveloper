...
//
//  Created by Shreya Pallan on 19/04/21.
...
class EssentialFeedAPIEndToEndTests: XCTestCase {
    ...
    func test_endToEndTestServerGETFeedResult_matchesFixedTestAccountData() {
        ...
        case let .success(imageFeed):
            ...
            XCTAssertEqual(imageFeed.count, 8,"Expected 8 images in the test account")
            
//            items.enumerated().forEach { (index,item) in
//                XCTAssertEqual(item, expectedItem(at:index),"Unexpected item values at index \(index)")
//            }
            
            /* if you have a FINITE number of items, use this insead of for loop to get the excact error*/
